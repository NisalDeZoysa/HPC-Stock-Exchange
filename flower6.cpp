...
bool allDemandsFulfilled(const std::vector<Buyer> &buyers)
{
    for (const Buyer &buyer : buyers)
    {
        ...
        {
            if (buyer.demand[f] > 0)
                return false; // found a demand not fulfilled yet
        }
    }
#pragma omp critical
    std::cout << "All buyers' demands have been fulfilled. Waiting for new orders...\n";
    return true; // all demands are zero
}
...
int main()
{
    ...
    // Flag to print demands fulfilled message once
    bool demandsAlreadyReported = false;
...
#pragma omp critical
                ...
                if (!(iss >> buyerName >> flowerType >> quantity) || flowerType < 0 || flowerType > 2 || quantity <= 0)
                {
#pragma omp critical
...
#pragma omp critical
                {
                    ...
                    for (auto &buyer : buyers)
                    {
                        if (buyer.name == std::string(buyerName))
...
#pragma omp critical
                {
                    ...
                    {
                        if (!demandsAlreadyReported)
                        {
                            std::cout << "All buyers' demands have been fulfilled. Waiting for new orders...\n";
                            demandsAlreadyReported = true;
                        }
                    }
                    else
                    {
                        demandsAlreadyReported = false;
                    }

                    if (!any_trade)
                    {
                        std::cout << "No trades in this round.\n";
                    }
                }
